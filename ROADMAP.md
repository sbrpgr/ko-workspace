...
- Current static asset cache version is `20260831-02`; bump this version whenever `app.js` or `styles.css` changes.
...
- The former advertising strip promotes Market, Money tips and Health: desktop three topic slots, mobile one interleaved topic/article, seven-second rotation, 50:50 image/text and five-minute feed refresh. Empty topics are skipped on mobile. No additional banner region or backend.
...
| 녹음 파일 간편 편집기 | `/tools/audio-editor/` | 녹음 파일 편집, 오디오 자르기, m4a 자르기, 휴대폰 녹음 편집 | M | Shipped browser-only waveform editor for common phone recordings such as m4a, aac, mp3, and wav when the browser can decode them. Supports mouse-wheel waveform zoom, horizontal scrolling for long recordings, selection delete/keep/copy/paste, undo/redo, volume adjustment, and WAV export without uploading recordings to the application server. |
...
| MD 파일 뷰어 | `/tools/markdown-viewer/` | MD 파일 뷰어, Markdown 뷰어, 마크다운 파일 열기, README 보기 | S | Shipped as the 30th browser-side tool. Opens `.md`, `.markdown`, and `.txt` files locally with font size, line height, theme, outline, source, and split views. The `새창에서 보기/편집` action opens an edit-only popout with automatic soft wrapping, copy, and save; the popout does not include a preview pane. Raw HTML is escaped before preview. |
