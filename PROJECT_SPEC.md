...
- Current static asset cache version: `20260501-11`
...
- `녹음 파일 텍스트 변환`
  ...
  - Defaults to the tested `onnx-community/whisper-tiny` stable profile after the larger browser model produced repeated hallucinated phrases on real recordings
  - Uses an automatic runtime path with a CPU compatibility option; unstable high-quality device/model choices are not exposed in the UI
