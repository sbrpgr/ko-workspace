...
- The recording-file transcription beta loads Transformers.js and a Whisper model only on demand; the selected recording file remains local to the browser. It now prefers WebGPU fp32 when available, keeps CPU mixed precision as the stable fallback, and remains a lightweight draft-text converter with an optional sentence-ending line break cleanup step, not a stored transcript manager.
