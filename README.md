...
- The recording-file transcription beta loads Transformers.js and a Whisper model only on demand; the selected recording file remains local to the browser. It now prefers fp32 quality paths first and falls back to a mixed precision compatibility path, remaining a lightweight draft-text converter with an optional sentence-ending line break cleanup step, not a stored transcript manager.
