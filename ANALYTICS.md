...
Insight Spirit cross-promotion (2026-08-31): Market, Money tips and Health outbound links use `utm_source=ko-workspace`, `utm_medium=referral`, `utm_campaign=spirit_latest`, and public article ID in `utm_content`. These parameters let each destination distinguish referred visits; no new Ko Workspace event is added. The three public article feeds omit credentials/referrer; image requests omit referrer. None contain input text, filenames, tool results or user identifiers.
